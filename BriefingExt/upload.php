...
try {
    ...
    if ((int) $file['size'] > 10 * 1024 * 1024) {
        throw new InvalidArgumentException('O anexo deve ter no máximo 10 MiB.');
    }
    $mime = (new finfo(FILEINFO_MIME_TYPE))->file($file['tmp_name']);
    $allowed = ['image/jpeg' => 'jpg', 'image/png' => 'png', 'image/webp' => 'webp', 'application/pdf' => 'pdf'];
    if (!isset($allowed[$mime])) {
        throw new InvalidArgumentException('Envie somente JPG, PNG, WebP ou PDF.');
    }
    ...
    error_log('[Briefing upload] ' . $e->getMessage());
    briefing_json(['ok' => false, 'message' => 'Não foi possível enviar o anexo.'], 500);
}
