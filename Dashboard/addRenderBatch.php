...
function adicionarDiasUteisEF($dataInicial, $diasUteis)
{
    $diasAdicionados = 0;
    ...
    while ($diasAdicionados < $diasUteis) {
        ...
        if (in_array(date('Y-m-d', $data), feriadosMoveis_EF($ano), true)) {
            continue;
        }
...
function feriadosMoveis_EF($ano)
{
...
// Status fixo EF
$statusId = 6;
$novoPrazo = adicionarDiasUteisEF($dataRecebimento, 7);
...
try {
    ...
    foreach ($ids as $imagemId) {
        ...
        if ($temFuncao && $funcaoIdExistente) {
            ...
            // Reset existing Alteração task to "Não iniciado"
            $stmtResetStatus->bind_param('i', $funcaoId);
        ...
        $stmtUpdateImagem->bind_param('isi', $statusId, $novoPrazo, $imagemId);
        $stmtUpdateImagem->execute();

        $descricao = trim($nomeImagem) . ' - Entrega EF';
        $tipoEvento = 'Entrega';
    ...
    echo json_encode([
        'status'    => 'sucesso',
        'message'   => 'EF adicionado com sucesso.',
