  ...
  fetch(`infosObra.php?obraId=${obraId}`)
    ...
    .then((data) => {
      ...
      document.getElementById("nomenclatura").textContent =
        obra.nomenclatura || "Nome não disponível";
      document.title = obra.nomenclatura || "Nome não disponível";
      document.getElementById("data_inicio_obra").textContent =
        `Data de Início: ${formatarData(obra.data_inicio)}`;
      document.getElementById("prazo_obra").textContent =
        `Prazo: ${formatarData(obra.prazo)}`;
      document.getElementById("dias_trabalhados").innerHTML =
        obra.dias_trabalhados
          ? `<strong>${obra.dias_trabalhados}</strong> dias`
          : "";
      document.getElementById("total_imagens").textContent =
        `Total de Imagens: ${obra.total_imagens}`;
      document.getElementById("total_imagens_antecipadas").textContent =
        `Imagens Antecipadas: ${obra.total_imagens_antecipadas}`;
...
function mostrarPorcentagem(funcaoSelecionada) {
  ...
  if (indexFuncao === -1) {
    console.error(
      `Função '${funcaoSelecionada}' não encontrada no array colunas.`,
    );
    return;
  }
...
function filtrarTabela() {
  var tipoImagemFiltro = document
    .getElementById("tipo_imagem")
    .value.toLowerCase();
  var antecipadaFiltro = document.getElementById("antecipada_obra").value;
  var statusEtapaImagemFiltro = document.getElementById(
    "imagem_status_etapa_filtro",
  ).value;
  var statusImagemFiltro = document.getElementById(
  ...
  for (var i = 0; i < linhas.length; i++) {
    ...
    var isAntecipada =
      linhas[i].querySelector("td[antecipada]").getAttribute("antecipada") ===
      "1";
    var statusEtapaColuna = linhas[i]
      .querySelector("td:nth-child(1)")
      .textContent.trim(); // Pegando o status da terceira coluna (ajuste conforme necessário)
    var statusColuna = linhas[i]
    ...
    if (
      tipoImagemFiltro &&
      tipoImagemFiltro !== "0" &&
      tipoImagemColuna.toLowerCase() !== tipoImagemFiltro.toLowerCase()
    ) {
      mostrarLinha = false;
    }
    ...
    if (
      statusImagemFiltro &&
      statusImagemFiltro !== "0" &&
      statusColuna !== statusImagemFiltro
    ) {
      mostrarLinha = false;
    }
    ...
    if (
      ...
      statusEtapaImagemFiltro !== "0" &&
      statusEtapaColuna !== statusEtapaImagemFiltro
    ) {
      mostrarLinha = false;
    }
...
function abrirModal(botao) {
  ...
  let containerFuncao =
    botao.closest(".funcao") || botao.closest(".funcao_comp");
  let nomeFuncao =
    containerFuncao?.querySelector(".titulo p")?.textContent.trim() || "";
...
function fecharModal() {
  ...
  document
    .querySelectorAll(".revisao_imagem")
...
function enviarArquivo() {
  ...
  xhr.upload.addEventListener("progress", (e) => {
    ...
    const remainingMB = e.total / (1024 * 1024) - uploadedMB;
...
function carregarPdf(nomeArquivo) {
  const nomenclatura = document
    .getElementById("nomenclatura")
    .textContent.trim();
  const url =
    "ver-pdf.php?arquivo=" +
    encodeURIComponent(nomeArquivo) +
    "&nomenclatura=" +
    encodeURIComponent(nomenclatura);
  pdfjsLib
    .getDocument(url)
