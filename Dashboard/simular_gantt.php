...
$obra_id = 52; // Altere para o ID da obra que deseja simular
...
function simularGantt($conn, $obra_id, $grupos)
{
    ...
    $datasEtapas = [];
    $data_atual = $imagensTodas[0]['recebimento_arquivos'] ?? date('Y-m-d');
    $colaborador_id = 19;
    ...
    while (count($fila) > 0) {
        ...
        foreach ($lote as $img) {
            foreach ($etapasConjunto as $etapa) {
    ...
    // Para a próxima etapa em conjunto, começa do mesmo ponto onde parou
    ...
    foreach (['Fachada', 'Imagem Externa'] as $grupoLote) {
        ...
        foreach ($imagensLote as $img) {
            $filtroFim = $datasEtapas[$img['id']]['Filtro de assets']['data_fim'] ?? null;
            if (!$filtroFim) {
    ...
    foreach ($gruposFiltrados as $grupo => $etapas) {
        ...
        foreach ($imagens as $img) {
            ...
            foreach ($etapas as $etapa => $dias) {
                ...
                // --- DEFINIÇÃO DO INÍCIO DE CADA ETAPA ---
                if ($etapa === 'Finalização' && in_array($grupo, ['Fachada', 'Imagem Externa'])) {
                    $data_inicio_etapa = $datasEtapas[$img['id']]['Modelagem']['data_fim'] ?? $datasEtapas[$img['id']]['Filtro de assets']['data_fim'] ?? $img['recebimento_arquivos'];
                } elseif ($etapa === 'Pós-Produção' && in_array($grupo, ['Fachada', 'Imagem Externa'])) {
                    $data_inicio_etapa = $datasEtapas[$img['id']]['Finalização']['data_fim'] ?? $datasEtapas[$img['id']]['Modelagem']['data_fim'] ?? $datasEtapas[$img['id']]['Filtro de assets']['data_fim'] ?? $img['recebimento_arquivos'];
                } elseif ($etapa === 'Modelagem') {
                    $data_inicio_etapa = $datasEtapas[$img['id']]['Filtro de assets']['data_fim'] ?? $img['recebimento_arquivos'];
                } else {
                    $data_inicio_etapa = $datasEtapas[$img['id']]['Finalização']['data_fim']
                        ?? $datasEtapas[$img['id']]['Composição']['data_fim']
                        ?? $datasEtapas[$img['id']]['Modelagem']['data_fim']
                        ?? $datasEtapas[$img['id']]['Filtro de assets']['data_fim']
                        ?? $img['recebimento_arquivos'];
                }

                // --- REGRAS DE COLABORADOR ---
                $idsPermitidos = null;
                if ($etapa === 'Finalização' && $grupo === 'Fachada') {
                    $idsPermitidos = [8, 20];
                } elseif (in_array($etapa, ['Caderno', 'Filtro de assets'])) {
                    $idsPermitidos = [19];
                } elseif ($etapa === 'Modelagem' && in_array($grupo, ['Fachada', 'Imagem Externa'])) {
                    $idsPermitidos = [16];
                    ...
                    $idsPermitidos = array_filter($idsPermitidos, function ($c) {
                        return $c !== 16;
                    });
...
function colaboradorDisponivel($conn, $etapa, $data_inicio, $dias, $idsPermitidos = null)
{
    ...
    if ($idsPermitidos) {
        $in = implode(',', array_map('intval', $idsPermitidos));
        $query .= " AND fc.colaborador_id IN ($in)";
    }
    $query .= " ORDER BY fc.colaborador_id ASC";
    $stmt = $conn->prepare($query);
    $stmt->bind_param("s", $etapa);
    $stmt->execute();
    $result = $stmt->get_result();
