...
CREATE TABLE IF NOT EXISTS review_batch_items (
    ...
    KEY idx_review_batch_items_p00_history (p00_versao_id, entered_rvw_at),
    KEY idx_review_batch_items_entrega_history (
        entrega_item_id,
        entered_rvw_at
    ),
    UNIQUE KEY ux_review_batch_p00_item_active (
        review_batch_id,
        p00_versao_id,
        imagem_id,
        item_active_slot
    ...
    CONSTRAINT fk_review_batch_items_p00_versao FOREIGN KEY (p00_versao_id) REFERENCES entregas_p00_versoes (id) ON DELETE CASCADE ON UPDATE CASCADE,
...
DROP PROCEDURE IF EXISTS sp_review_batch_enter_drv_p00 $$
...
DROP PROCEDURE IF EXISTS sp_review_batch_leave_drv_p00 $$
