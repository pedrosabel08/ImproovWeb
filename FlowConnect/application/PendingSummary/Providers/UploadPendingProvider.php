...
final class UploadPendingProvider extends AbstractPendingSummaryProvider {
    public function __construct(array $config) { parent::__construct($config, 'arquivo', 'Arquivos', 'PaginaPrincipal/'); }
    public function collect(mysqli $conn): array {
        ...
        foreach($rows as &$r){$r['entity_type']='funcao_imagem';$r['title']=trim(($r['imagem']?:'Imagem').' · '.($r['funcao']?:'Função não identificada').' · arquivo pendente');$r['priority']=$this->priority($r['due_at']??null);$r['link']='PaginaPrincipal/';}unset($r);return $this->summarize($rows);
    }
}
