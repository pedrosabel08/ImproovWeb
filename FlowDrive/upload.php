...
$descricao = $_POST['descricao'] ?? "";
$sufixo = $_POST['sufixo'] ?? "";
$sufixos_por_arquivo = $_POST['sufixo_por_arquivo'] ?? []; // per-file sufixos (geral multi-upload)
$sufixo_global = $sufixo; // preserved for fallback
...
if (!empty($arquivosTmp) && count($arquivosTmp) > 0 && ($refsSkpModo === 'geral' || $tipo_arquivo !== 'SKP')) {
    ...
    foreach ($arquivosTmp as $index => $fileTmp) {
        // Use per-file sufixo when provided (multi-file geral upload), else global
        $sufixo = !empty($sufixos_por_arquivo) && isset($sufixos_por_arquivo[$index])
            ? trim($sufixos_por_arquivo[$index])
            : $sufixo_global;
