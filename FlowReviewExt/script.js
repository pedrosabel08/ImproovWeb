...
document.addEventListener("DOMContentLoaded", async function () {
  try {
    ...
    if (!auth.authenticated) {
      ...
      try {
        localStorage.setItem("idusuario", String(auth.idusuario || ""));
      } catch (e) {}
      hideAuthOverlay();
  ...
  if (loginForm) {
    loginForm.addEventListener("submit", async function (e) {
      ...
      try {
        ...
        if (json.success) {
          if (json.idusuario)
            localStorage.setItem("idusuario", String(json.idusuario));
          Toastify({
          ...
          Toastify({
            text: json.message || "Credenciais inválidas",
            duration: 4000,
            backgroundColor: "red",
            close: true,
            gravity: "top",
        ...
        Toastify({
          text: "Erro ao contatar o servidor. Tente novamente.",
          duration: 4000,
          backgroundColor: "red",
          close: true,
          gravity: "top",
    ...
    registerForm.addEventListener("submit", async function (e) {
      ...
      try {
        ...
        if (json.success) {
          ...
          Toastify({
            text: json.message || "Falha ao registrar",
            duration: 4000,
            backgroundColor: "red",
            close: true,
            gravity: "top",
...
async function fetchEntregasObraFixa() {
  ...
  const url = token
    ? `atualizar.php?token=${encodeURIComponent(token)}`
    : "atualizar.php";
  ...
  entregas.forEach((ent) => {
    ...
    (ent.itens || []).forEach((item) => {
      ...
      if (item.idfuncao_imagem)
        grupo.funcao_imagem_ids.add(item.idfuncao_imagem);
    });
  });
  const resultado = [];
  mapa.forEach((grupo) => {
    ...
    grupo.preview_url =
      (grupo.versoes[0] && grupo.versoes[0].imagem_url) || null;
...
function exibirGridImagens(imagens) {
  ...
  if (imagens.length === 0) {
    container.innerHTML =
      '<p style="text-align:center;color:#888;">Nenhuma imagem encontrada.</p>';
    return;
  }
  imagens.forEach((img) => {
  ...
  if (
    (
      await FlowAlert.confirm({
        ...
        message: `Você tem certeza de que deseja ${actionText}?`,
...
async function buscarMencoesDoUsuario() {
  const r = await fetch("buscar_mencoes.php");
  return await r.json();
}
...
async function exibirCardsDeObra(tarefas) {
  ...
  tarefas.forEach((t) => {
    if (!obrasMap.has(t.nome_obra)) obrasMap.set(t.nome_obra, []);
    obrasMap.get(t.nome_obra).push(t);
  });
  obrasMap.forEach((tarefasDaObra, nome_obra) => {
    ...
    const imagemPreview = tarefaComImagem
      ? `https://improov.com.br/flow/ImproovWeb/${tarefaComImagem.imagem}`
      : "../assets/logo.jpg";
...
function filtrarTarefasPorObra(obraSelecionada) {
  ...
  if (tarefasDaObra.length > 0) {
    ...
    document.querySelectorAll(".obra_nav").forEach((link) => {
...
function atualizarFiltrosDinamicos(tarefas) {
  ...
  colaboradores.forEach((col) => {
    ...
    o.value = col;
    o.textContent = col;
    selectColaborador.appendChild(o);
  });
  if (
  ...
  if (tarefas.length > 0) {
    tarefas.forEach((tarefa) => {
      ...
      const imagemPreview = tarefa.imagem
        ? `https://improov.com.br/flow/ImproovWeb/${tarefa.imagem}`
        : "../assets/logo.jpg";
      const color =
...
function construirLabelVersao(v) {
  console.log(v);
  const envio =
    v.indice_envio !== undefined && v.indice_envio !== null
      ? `${v.nome_etapa}`
      : "Envio ?";
...
function historyAJAX(imagemId) {
  ...
  if (selectVersoes) {
    ...
    versoesOrdenadas.forEach((v, idx) => {
      ...
      opt.textContent =
        idx === 0 ? construirLabelVersao(v) : construirLabelVersao(v);
    ...
    selectVersoes.onchange = function () {
      ...
      const versao = versoesAtuaisDaImagem.find(
        (v) => String(v.historico_imagem_id) === String(idHist),
      );
      if (versao) {
        mostrarImagemCompleta(
          versao.imagem_url,
  ...
  if (imageContainer) {
    ...
    if (entregaReferencia !== null) {
      gruposDaEntrega = imagensAgrupadasGlobal.filter((gr) =>
        (gr.versoes || []).some(
          (v) => (v.entrega_id || null) === entregaReferencia,
        ),
      );
    }
    ...
    gruposDaEntrega.forEach((gr) => {
      ...
      caption.textContent =
        ultimaVersao.imagem_nome || `Imagem ${gr.imagem_id}`;
...
function mostrarImagemCompleta(src, id, imagem_nome = "") {
  ...
  const versaoCorrente =
    versoesAtuaisDaImagem.find(
      (v) => String(v.historico_imagem_id) === String(id),
    ) || versoesAtuaisDaImagem[0];
  ...
  const statusIsP00 =
    versaoCorrente &&
    (versaoCorrente.nome_status_imagem === "P00" ||
      versaoCorrente.nome_etapa === "P00");
  const shouldLoadCarousel =
    versaoCorrente &&
    (versaoCorrente.carrossel_angulos ||
      (hasAngles && isNoHistorico && statusIsP00));

  const imagemCompletaContainer =
    document.getElementById("imagem_completa") || imageWrapper;
  ...
  if (shouldLoadCarousel && entregaItemIdAtual) {
    ...
    carregarCarrosselAngulos(
      entregaItemIdAtual,
      imagemCompletaContainer,
      imgElement,
    );
  } else {
    ...
    try {
      document
        .querySelectorAll(".angle-decision-info")
        .forEach((el) => el.remove());
    } catch (e) {}
    try {
      document
        .querySelectorAll(".carrossel-actions")
        .forEach((el) => el.remove());
    } catch (e) {}
    try {
      document.body.classList.remove("has-angle-carousel");
    } catch (e) {}
  ...
  const nomeSpan = document.getElementById("imagem_nome");
  if (nomeSpan) nomeSpan.textContent = imagem_nome;
}

// Busca ângulos e monta carrossel abaixo da imagem principal
async function carregarCarrosselAngulos(
  entregaItemId,
  containerWrapper,
  mainImgEl,
) {
  try {
    const r = await fetch(
      `get_angulos_imagem.php?entrega_item_id=${encodeURIComponent(entregaItemId)}`,
    );
    ...
    try {
      document.body.classList.add("has-angle-carousel");
    } catch (e) {}
    ...
    const decidedAngle = j.angulos.find(
      (a) => String(a.decisao).toLowerCase() === "aprovado",
    );
    ...
    function selectThumb(i) {
      ...
      try {
        window.selectedAngleId = t.dataset.anguloId
          ? String(t.dataset.anguloId)
          : null;
      } catch (e) {
        window.selectedAngleId = null;
      }
      ...
      if (imgEl && imgEl.dataset.preview) {
        ...
        const currentMain =
          document.getElementById("imagem_atual") || mainImgEl;
        try {
          currentMain.src = imgEl.dataset.preview;
        } catch (e) {
          /* ignore */
        }
      }
      ...
      try {
        ...
        const anyAngleSelected =
          (typeof window.selectedAngleId === "string" &&
            window.selectedAngleId !== "") ||
          (decidedIndex !== null && i === decidedIndex);
        ...
        if (submitBtnLocal) {
          if (anyAngleSelected) {
            ...
            const allowed = [1, 2, 3, 9, 20];
            submitBtnLocal.style.display = allowed.includes(idusuario)
              ? ""
              : "none";
          }
        }
        ...
        if (refazerBtn) {
          try {
            refazerBtn.style.display = anyAngleSelected ? "none" : "";
          } catch (e) {}
        }
      } catch (err) {
      ...
      try {
        ...
        function placeInfoBoxNearSubmit() {
          ...
          if (sb) {
            ...
            try {
              sb.insertAdjacentElement("afterend", angleDecisionInfoBox);
            } catch (e) {
    ...
    if (decidedIndex === null) {
      try {
        ...
        refazerBtn.addEventListener("click", (e) => {
          ...
          if (ta) ta.value = "";
          if (modal) {
            modal.dataset.entregaItemId = String(
              entregaItemId || entregaItemIdAtual || "",
            );
        ...
        try {
          header.insertAdjacentElement("afterend", actionsDiv);
        } catch (e) {
          carrossel.appendChild(actionsDiv);
        }
      } catch (e) {
        /* ignore UI errors */
      }
    ...
    try {
      containerWrapper.style.position =
    ...
    faixa.addEventListener("pointerup", (e) => {
      isDown = false;
      try {
        faixa.releasePointerCapture(e.pointerId);
      } catch (err) {}
      ...
      if (!movedDuringDrag) {
        ...
        if (lastClickedIndex !== null) {
          try {
            selectThumb(lastClickedIndex, true);
          } catch (err) {
            /* ignore */
          }
          lastClickedIndex = null;
...
function ensureRefazerModal() {
  ...
  document
    ...
    .addEventListener("click", async () => {
      ...
      if (!entregaId) {
        Toastify({
          text: "ID da entrega não disponível.",
          duration: 3000,
          backgroundColor: "red",
          close: true,
          gravity: "top",
      ...
      try {
        ...
        if (j.success) {
          Toastify({
            text: j.message || "Solicitação enviada. A gestão será notificada.",
            duration: 4000,
            backgroundColor: "green",
            close: true,
            gravity: "top",
          ...
          Toastify({
            text: j.message || "Falha ao enviar solicitação.",
            duration: 4000,
            backgroundColor: "red",
            close: true,
            gravity: "top",
        ...
        Toastify({
          text: "Erro na comunicação. Tente novamente.",
          duration: 4000,
          backgroundColor: "red",
          close: true,
          gravity: "top",
...
async function renderDecisions(historicoId, entregaItemId) {
  try {
    ...
    if (!j.success) {
      cont.innerHTML =
        '<div class="decisoes-empty">Erro ao carregar decisões.</div>';
      return;
    }
    ...
    list.forEach((d) => {
      ...
      const label =
        d.decisao === "aprovado"
          ? "Aprovado"
          : d.decisao === "aprovado_com_ajustes"
  ...
  Object.entries(tarefasPorFuncao).forEach(([funcao, tarefas]) => {
    ...
    tarefas.forEach((t) => {
      ...
      const imgSrc = t.imagem
        ? `https://improov.com.br/flow/ImproovWeb/${t.imagem}`
        : "../assets/logo.jpg";
      tarefa.innerHTML = `
    ...
    if (!USERS_PERMITIDOS_DECISAO.includes(idusuario)) {
      ...
      if (btnOpen) {
        btnOpen.addEventListener("click", () => {
          ...
          if (
            !entregaItemIdAtual ||
            (!ap_imagem_id && !window.selectedAngleId)
          ) {
            Toastify({
              text: "Selecione uma imagem/versão válida antes.",
              duration: 3000,
              backgroundColor: "orange",
              close: true,
              gravity: "top",
      ...
      if (btnConfirm)
        btnConfirm.addEventListener("click", async () => {
          ...
          try {
            ...
            const observacaoVal = obsEl
              ? String(obsEl.value || "").trim() || null
              : null;
            ...
            if (json.success) {
              ...
              Toastify({
                text: json.message || "Falha ao registrar decisão.",
                duration: 3000,
                backgroundColor: "red",
                close: true,
                gravity: "top",
