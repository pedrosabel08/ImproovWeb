  ...
  function renderAttention(items) {
    ...
    list.innerHTML = items.length
      ? items
          ...
          .map(
            (item) =>
              `<a class="attention-card severity-${escape(item.severity || "info")}" href="${escape(target(item.cta))}"><span class="attention-icon"></span><span><h3>${escape(item.title || item.imagem?.nome || "Ação necessária")}</h3><p>${escape(item.description || "Revise esta situação.")}</p></span></a>`,
          )
          .join("")
  ...
  function renderCurrent(task) {
    ...
    if (!task) {
      node.innerHTML = empty(
        "Nenhuma atividade operacional recomendada agora.",
      );
      return;
    }
    ...
    const preview = task.preview_url
      ? `<div class="current-work__preview"><img src="${escape(task.preview_url)}" alt=""><span class="current-work__project">${escape(task.obra?.nome || "Projeto")}</span></div>`
      : `<div class="current-work__preview"><span class="current-work__project">${escape(task.obra?.nome || "Projeto")}</span></div>`;
  ...
  function renderNext(tasks) {
    ...
    node.innerHTML = tasks.length
      ? tasks
          ...
          .join("")
      : empty("Nenhuma próxima tarefa elegível.");
  }
