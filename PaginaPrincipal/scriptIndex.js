...
function updateKanbanLoadingSignature(stage, retry = false) {
  const signature = ensureKanbanLoadingSignature();
  if (!signature) return;
...
function setKanbanLoadingCounts() {
  document.querySelectorAll(".kanban-box .task-count").forEach((badge) => {
    badge.textContent = "";
    badge.classList.add("task-count--loading");
    badge.setAttribute("aria-label", "Carregando quantidade de tarefas");
  });
}
...
function renderKanbanSkeletons() {
  const board = getKanbanBoard();
  if (!board) return;
  board.querySelectorAll(".kanban-box").forEach((box) => {
...
function setKanbanLoadingState(state) {
  const board = getKanbanBoard();
  if (!board) return;
...
function showKanbanLoading(loadId, initial) {
  ...
  kanbanLoading.shownAt = performance.now();
  const board = getKanbanBoard();
  if (!board) return;
  if (initial) {
    board.classList.add("kanban--loading");
    renderKanbanSkeletons();
    updateKanbanLoadingSignature("loading");
  } else {
    board.classList.add("kanban--refreshing");
    setKanbanRefreshingCounts();
    updateKanbanLoadingSignature("refreshing");
  }
}
...
function showKanbanLoadError(loadId, initial, error) {
  if (loadId !== kanbanLoading.sequence) return;
  ...
  updateKanbanLoadingSignature("error", true);
}
...
function renderKanbanResponse(loadId, data) {
  ...
  window.setTimeout(() => {
    ...
    window.setTimeout(() => {
      if (loadId !== kanbanLoading.sequence) return;
      clearKanbanLoadingVisuals();
      setKanbanLoadingState("READY");
    }, 220);
  }, waitForSkeleton);
}
...
function carregarDados(colaborador_id) {
  ...
  const fail = (error) => {
    if (settled) return;
    settled = true;
    showKanbanLoadError(loadId, initial, error);
  };
...
function formatarDataPlanejamento(data) {
  return data ? formatarData(data) : "Não definido pelo planejamento";
}

function resumoPlanejamentoKanban(contexto) {
  const prazo = contexto?.prazo_necessario;
  const temporal = contexto?.status_temporal || {};
  if (!prazo) {
...
function abrirFormularioFlowBlockHold(
  ...
  const permaneceNaoIniciado =
    String(card.dataset.status || "").trim() === "Não iniciado";
  ...
  const origemTarefa = requirement?.origem_tarefa || null;
  const origemResponsavel =
    origemTarefa?.responsavel_nome ||
    requirement?.origem_responsavel_nome ||
    "";
  const origemTarefaTexto = origemTarefa?.nome
    ? `${origemTarefa.nome}${origemTarefa.imagem_nome ? ` · ${origemTarefa.imagem_nome}` : ""}`
    : "";
  const contextoRequisito = requirement
    ? `<div class="flow-block-hold-context">
         ...
         <dl>
           ...
           ${origemTarefaTexto ? `<div><dt>Tarefa anterior/origem</dt><dd>${escapeKanbanText(origemTarefaTexto)}</dd></div>` : ""}
           ${origemResponsavel ? `<div><dt>Responsável</dt><dd>${escapeKanbanText(origemResponsavel)}</dd></div>` : ""}
  ...
  const descricaoInicial = requirement
    ? [
        ...
        origemTarefaTexto ? `Tarefa anterior/origem: ${origemTarefaTexto}` : "",
        origemResponsavel ? `Responsável: ${origemResponsavel}` : "",
        requirementContext,
      ]
        .filter(Boolean)
        .join("\n")
  ...
  form.innerHTML = `
    ...
    <div class="flow-block-hold-grid">
      <label>Fila responsável <select data-field="fila_id" required><option value="">Selecione a fila</option></select></label>
      <label>Responsável <select data-field="responsavel_id" required><option value="">Selecione o responsável</option></select></label>
  ...
  const fillSelect = (
    ...
    select.innerHTML =
      ...
      items
        ...
        .join("");
    const suggestedItem = suggestedCode
      ? items.find(
          (item) => String(item.codigo || "").toUpperCase() === suggestedCode,
        )
      : items.find((item) => Number(item.id) === Number(suggestedValue));
    if (suggestedItem) select.value = String(suggestedItem.id);
  };
  ...
  fetch("FlowBlock/api.php?action=options")
    ...
    .then((data) => {
      ...
      const sugestao = requirement?.flow_block_sugestao || {};
      fillSelect(
        "tipo_id",
        data.types || [],
        "Selecione o tipo",
        null,
        sugestao.tipo_codigo || "",
      );
      fillSelect(
        "fila_id",
        data.queues || [],
        "Selecione a fila",
        null,
        sugestao.fila_codigo || "",
      );
      fillSelect(
        "responsavel_id",
        data.collaborators || [],
        "Selecione o responsável",
        sugestao.responsavel_id || null,
      );
    })
    .catch((error) => {
  ...
  form
    ...
    .addEventListener("click", async () => {
      ...
      const queueId = Number(
        form.querySelector('[data-field="fila_id"]').value,
      );
      ...
      if (!typeId || !queueId || !responsibleId || !description) {
        Toastify({
          text: "Informe o tipo, a fila responsável, o responsável e a observação do Impedimento.",
          duration: 3000,
          gravity: "top",
          position: "left",
          backgroundColor: "#d94b4b",
      ...
      try {
        const response = await fetch("FlowBlock/api.php?action=create", {
          ...
          body: JSON.stringify({
            ...
            requirement_origin_id: Number(requirement?.origem_id || 0) || null,
...
function formatarTempoFlowBlock(data) {
  ...
  const minutos = Math.max(
    0,
    Math.floor((Date.now() - inicio.getTime()) / 60000),
  );
...
function abrirReplanejamentoFlowBlock(card) {
  ...
  const prazoData = prazoAnterior
    ? new Date(`${prazoAnterior}T00:00:00`)
    : null;
  const prazoVencido =
    prazoData && prazoData.getTime() < new Date().setHours(0, 0, 0, 0);
  ...
  form
    ...
    .addEventListener("click", async () => {
      ...
      if (!deadline) {
        Toastify({
          text: "Informe o novo prazo para continuar a tarefa.",
          duration: 3000,
          gravity: "top",
          position: "left",
          backgroundColor: "#d94b4b",
      ...
      try {
        ...
        if (!response.ok || !data.ok)
          throw new Error(
            data.message || "Não foi possível continuar a tarefa.",
          );
        close();
        carregarDados(colaborador_id);
        Toastify({
          text: "Tarefa reprogramada e movida para Em andamento.",
          duration: 3500,
          gravity: "top",
          position: "left",
...
function processarDados(data) {
  ...
  function criarCard(item, tipo, media) {
    ...
    // A urgência visível vem do prazo necessário. O prazo legado continua
    // disponível somente para fluxos sem correspondência no planejamento.
    const atrasada =
      tipo === "imagem" && planejamento?.planejamento_disponivel
        ? planejamento?.status_temporal?.codigo === "ATRASADO"
        : status === "Em andamento" && item.prazo
          ? isAtrasada(item.prazo)
          : false;
    ...
    const resumoPendenciasInicio =
      status === "Não iniciado" && pendenciasInicio.length > 0
        ? `<div class="requirements-card-summary">
             <strong><i class="ri-forbid-2-line"></i> ${pendenciasInicio.length} ${pendenciasInicio.length === 1 ? "dependência impede" : "dependências impedem"} o início</strong>
             <button type="button" class="kanban-dependencies-toggle">Ver dependências <i class="ri-arrow-right-s-line"></i></button>
           </div>`
        : status === "Não iniciado" && liberado === "1"
          ? `<div class="requirements-card-summary is-ready"><strong><i class="ri-checkbox-circle-line"></i> Pronta para iniciar</strong></div>`
          : "";
    const diasDesvioPlanejamento = Number(
      planejamento?.status_temporal?.dias ?? 0,
    );
    const desvioPlanejamentoHtml =
      tipo === "imagem" &&
      status === "Não iniciado" &&
      planejamento?.prazo_necessario &&
      diasDesvioPlanejamento > 0
        ? `<strong class="kanban-planning-deviation">+${diasDesvioPlanejamento}d</strong>`
        : "";

    const flowBlockIssueCount = Number(item.flow_block_issues_abertas || 0);
    const flowBlockPrincipalStatus = String(
      item.flow_block_issue_principal_status || "",
    );
    const flowBlockAwaitingConfirmation =
      flowBlockPrincipalStatus === "RESOLVIDA";
    const flowBlockPaused = flowBlockPrincipalStatus === "PAUSADA";
    const flowBlockReadyToContinue =
      Number(item.flow_block_pronta_para_continuar || 0) === 1;
    const flowBlockIssueHtml = flowBlockReadyToContinue
    ...
    if (tempoClass === "tempo-ruim")
      card.classList.add("kanban-card--sla-critical");
    ...
    if (status === "Não iniciado") {
      card.querySelector(".card-log")?.remove();
      card.classList.remove("kanban-card--with-thumb");
      card.querySelector(".kanban-card-thumb")?.remove();
    }
    ...
    card.dataset.planningAvailable = planejamento.planejamento_disponivel
      ? "1"
      : "0";
    ...
    card.dataset.planningDifference =
      planejamento.diferenca_previsao_dias_uteis ?? "";
    ...
    card.dataset.flowBlockReadyToContinue = flowBlockReadyToContinue
      ? "1"
      : "0";
    ...
    const flowBlockContinueTask = card.querySelector(
      ".flow-block-continue-task",
    );
  ...
  document.querySelectorAll(".kanban-box .content").forEach((column) => {
    Array.from(column.querySelectorAll(":scope > .kanban-card"))
      .slice(0, 8)
      .forEach((card, index) => {
...
function abrirSidebar(
  ...
  return fetch(
    ...
    .then((data) => {
        ...
        <div class="tp-header-main">
          <div class="tp-title-block">
            ...
            <div class="tp-meta-row">
              ...
              <div class="tp-meta-sep"></div>
              <div class="tp-meta-item">
                <strong>Prazo necessário</strong>
                <span>${formatarDataPlanejamento(prazoNecessario)}</span>
              </div>
            </div>
          </div>
      ...
      const compromisso = document.createElement("section");
      compromisso.className = `tp-card tp-commitment-card is-${String(statusTemporal.codigo || "sem_prazo").toLowerCase()}`;
      const timelineItens = Array.isArray(planejamento.timeline)
      ...
      compromisso.innerHTML = `
        <div class="tp-commitment-header">
          <div><span class="tp-section-label"><i class="ri-calendar-check-line"></i> Compromisso da etapa</span><p>Seus prazos e o contexto do processo para esta etapa.</p></div>
          <strong class="tp-commitment-status"><i class="ri-time-line"></i> ${escapeKanbanText(statusTemporal.rotulo || "Prazo não definido")}</strong>
        </div>
        <div class="tp-commitment-grid">
      ...
      if (
        ...
        requisitosCard.id = "tp-dependencies-section";
                  ...
                  estado === "ATENDIDO"
                    ...
                    : estado === "NAO_ATENDIDO"
                      ...
                      : estado === "DISPENSADO"
                        ? "Dispensado"
                        : "Não aplicável";
                const origemTarefa = requisito.origem_tarefa || null;
                const origemDetalhe = origemTarefa?.nome
                  ? ` · ${origemTarefa.nome}${origemTarefa.responsavel_nome ? ` · ${origemTarefa.responsavel_nome}` : ""}`
                  : requisito.origem_responsavel_nome
                    ? ` · ${requisito.origem_responsavel_nome}`
                    : "";
                return `
                  <div class="tp-requirement-row state-${estado.toLowerCase()}">
                    ...
                    <span>
                      ...
                      <small>${escapeKanbanText(requisito.tipo || "")} · ${escapeKanbanText(requisito.origem || "")}${escapeKanbanText(origemDetalhe)}</small>
                    </span>
                    <em>${statusLabel}</em>
                    <div class="tp-requirement-actions">${flowBlockAction}</div>
                  </div>`;
              })
              .join("")}
          </div>
...
function preencherFiltros() {
  ...
  [...statuses]
    .sort((a, b) => {
      ...
      const ordemA = posicaoA === -1 ? ordemStatus.length : posicaoA;

      const ordemB = posicaoB === -1 ? ordemStatus.length : posicaoB;
  ...
  filtroObra.querySelectorAll('input[type="checkbox"]').forEach((checkbox) => {
    if (checkbox.value && obrasSelecionadas.has(checkbox.value)) {
  ...
  filtroFuncao
    ...
    .forEach((checkbox) => {
      if (checkbox.value && funcoesSelecionadas.has(checkbox.value)) {
        checkbox.checked = true;
      }
    });
  ...
  filtroStatus
    .querySelectorAll('input[type="checkbox"]')
    .forEach((checkbox) => {
      if (checkbox.value && statusSelecionados.has(checkbox.value)) {
        checkbox.checked = true;
      }
    });
...
function configurarModalPlanejamento(card) {
  if (!modalPlanejamento || !card || card.classList.contains("tarefa-criada")) {
...
document.getElementById("salvarModal").addEventListener("click", async () => {
  ...
  if (!cardSelecionado.classList.contains("tarefa-criada")) {
    try {
      ...
      Toastify({
        text: error?.message || "Não foi possível salvar sua previsão.",
        duration: 4000,
        close: true,
        gravity: "top",
        position: "left",
...
if (typeof Sortable !== "undefined") {
    ...
    new Sortable(col, {
      ...
      `,
        ...
        if (deColuna.id !== novaColuna.id) {
          ...
          if (_modalTitleDrag) {
            _modalTitleDrag.textContent = card.classList.contains(
              "tarefa-criada",
            )
              ? "Editar Card"
              : titulo || "Editar Card";
  ...
  console.warn(
    "Sortable não foi carregado; o arrastar e soltar está indisponível.",
  );
}
