...
function processarDados(data) {
  ...
  function criarCard(item, tipo, media) {
    ...
    else if (status === "Em aprovação") status = "Em aprovação";
    else if (status === "Em andamento") status = "Em andamento";
    else if (
      ["Aprovado", "Aprovado com ajustes", "Finalizado"].includes(status)
    )
      status = "Finalizado";
    else if (status === "Não iniciado") status = "Não iniciado";
    else if (status === "HOLD" || status === "Hold") status = "HOLD";
          ...
          document
            ...
            .addEventListener("submit", function onSubmit(e) {
              ...
              fetch("submit_respostas.php", {
                ...
                .then((data) => {
                  if (data.success) {
                    document.getElementById("modalDaily").style.display =
                      "none";
                    Swal.fire({
                      icon: "success",
                      ...
                      if (typeof checkFuncoesEmAndamento === "function") {
                        checkFuncoesEmAndamento(idColaborador).catch((err) =>
                          console.error(
          ...
          Swal.fire({
            ...
            if (result.isConfirmed) {
              ...
              FlowAlert.input({
                ...
                field: {
                  type: "text",
                  placeholder: "Por que não está fazendo?",
                },
                confirmText: "Salvar",
              }).then((obsResult) => {
...
function atualizarTaskCount() {
  Object.keys(statusMap).forEach((status) => {
    ...
    const count = col.querySelectorAll(
          ...
          if (novo === "em andamento" && prev === "aprovado com ajustes") {
            ...
            abrirSidebar(dados.cardId, dados.imagem_id)
              .then((data) => {
                ...
                let notificacoesDiv = sidebarContent.querySelector(
...
function enviarImagens() {
  ...
  xhr.onreadystatechange = () => {
    if (xhr.readyState !== 4 || uploadCancelado) return;
    try {
      ...
      if (res.error) {
        ...
        window.setTimeout(uploadProgress.close, 1800);
      } else {
        uploadProgress.update({
          title: "Prévia enviada com sucesso!",
          message: "O arquivo foi recebido.",
          progress: 100,
          type: "success",
        });
...
function enviarArquivo() {
  ...
  xhr.onload = () => {
    ...
    if (xhr.status >= 200 && xhr.status < 300) {
      uploadProgress.update({
        title: "Arquivo enfileirado",
        message: "O envio continuará em segundo plano.",
        progress: 100,
        type: "success",
      });
      window.setTimeout(uploadProgress.close, 1600);
      cardModal.classList.remove("active");
      cardSelecionado = null;
      try {
        const id = Array.isArray(res) && res[0]?.id ? res[0].id : null;
        if (id) window.subscribeUploadProgress?.(id, function (payload) {});
      } catch (e) {
