...
try {
    ...
    if (!improov_usuario_eh_gestor_sidebar($conn)) {
        ...
        throw new RuntimeException('Somente gestores podem validar capacidade excepcional.');
    }
    $conn->begin_transaction();
    try {
        $resultado = flow_alocacao_validar_capacidade_excepcional(
            $conn,
            $inicio,
            $fim,
            $payload,
    ...
    $codigo = str_contains($erro->getMessage(), 'mudou') ? 409 : 422;
