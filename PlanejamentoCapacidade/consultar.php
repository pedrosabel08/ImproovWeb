...
require_once __DIR__ . '/../helpers/planejamento_capacidade_global_helper.php';
...
try {
    ...
    if (!flow_capacidade_data_valida($inicio) || !flow_capacidade_data_valida($fim) || $fim < $inicio) {
        ...
        throw new InvalidArgumentException('Informe inicio e fim válidos no formato Y-m-d.');
    }

    $conn = conectarBanco();
    if (!improov_usuario_eh_gestor_sidebar($conn)) {
        http_response_code(403);
        throw new RuntimeException('Somente gestores podem consultar a capacidade global.');
    }
    ...
    if (!empty($_GET['etapa'])) {
        $opcoes['etapa'] = strtoupper(trim((string) $_GET['etapa']));
    }
    $resultado = flow_capacidade_consultar($conn, $inicio, $fim, $opcoes);
    $conn->close();
    echo json_encode(['success' => true, 'capacidade' => $resultado], JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
