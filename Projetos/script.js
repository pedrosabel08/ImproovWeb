...
document.addEventListener("DOMContentLoaded", () => {
  tabela = new Tabulator("#tabelaGestaoImagens", {
    ...
    ajaxResponse: function (url, params, response) {
      ...
      const etapasUnicas = [
        ...new Set(dados.map((item) => item.nome_status_imagem || "Sem etapa")),
      ].sort();
      const etapasUnicasObj = Object.fromEntries(
        etapasUnicas.map((v) => [v, v]),
      );
      const statusUnicos = [
        ...new Set(dados.map((item) => item.situacao)),
      ].sort();
      const statusUnicosObj = Object.fromEntries(
        statusUnicos.map((v) => [v, v]),
      );
      const tiposUnicos = [
        ...new Set(dados.map((item) => item.tipo_imagem || "Não definido")),
      ].sort();
    ...
    groupHeader: function (value, count, data, group) {
      ...
      if (field === "obra") {
        ...
        if (value && value !== "0000-00-00") {
          const todasEntregues = data.every(
            (item) => item.situacao === "RVW" || item.situacao === "DRV",
          );
  ...
  // O Tabulator redesenha o grupo ao expandir/recolher. Como essa página
  // usa a rolagem do documento, preserve a posição antes desse redesenho.
  const tabelaElement = document.getElementById("tabelaGestaoImagens");
  tabelaElement?.addEventListener(
    "click",
    (event) => {
      const target = event.target instanceof Element ? event.target : null;
      const groupHeader = target?.closest(".tabulator-group");
      if (
        !groupHeader ||
        !groupHeader.querySelector(".tabulator-group-toggle, .tabulator-arrow")
      ) {
        return;
      }
  ...
  function applyFilterAndPreventScroll(
    filterField,
    filterValue,
    callback = null,
  ) {
    // A página usa a rolagem do documento; preserve essa posição enquanto
    // o Tabulator redesenha as linhas após a aplicação do filtro.
...
function atualizarIndicadores(ind) {
  ...
  document.getElementById("totalAtrasadas").textContent =
    `Atrasadas: ${ind.atrasadas}`;
  document.getElementById("totalPrazoHoje").textContent =
    `Prazo Hoje: ${ind.prazo_hoje}`;
  document.getElementById("totalImagens").textContent =
    `Total imagens: ${ind.total}`;
}
