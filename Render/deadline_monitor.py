...
DEADLINE_JOB_IDS = [s.strip() for s in os.getenv("DEADLINE_JOB_IDS", "").split(",") if s.strip()]
try:
...
def run_deadline_command(args):
    ...
    if result.returncode != 0:
        log_and_print(f"❌ deadlinecommand retornou código {result.returncode}: {' '.join(command)}", "error")
        if output:
...
def delete_deadline_job(job_id):
    result, output = run_deadline_command(["DeleteJob", job_id])
    if result:
...
def extract_render_output(job_data, raw_output):
    render_output = clean_render_output_value(first_value(job_data, ("RenderOutput",), ""))
    if render_output:
...
def deadline_status_to_legacy(job_data, task_data, render_output):
    status = str(first_value(job_data, ("Status", "JobStatus"), "") or "").strip().lower()
    task_statuses = [str(v).strip().lower() for v in values_list(task_data, "TaskStatus")]
    ...
    active = not complete and status not in {"failed", "suspended", "deleted", "archived"}

    return {
        "Active": "Yes" if active else "No",
        "Complete": "Yes" if complete else "No",
        "Computer": first_value(task_data, ("TaskSlaveName", "SlaveName", "WorkerName", "MachineName"))
                    or first_value(job_data, ("MachineName", "WorkerName")),
        "Name": first_value(job_data, ("Name", "JobName")),
        "Submitted": first_value(job_data, ("SubmitDateTime", "SubmitDate", "JobSubmitDateTime")),
        "Description": first_value(job_data, ("Description", "Comment", "JobComment", "ExtraInfo0")),
        "LastUpdated": first_value(job_data, ("CompletedDateTime", "LastWriteTime", "LastUpdatedDateTime", "LastUpdated")),
...
def deadline_error_info(job_id, job_data, task_data):
    ...
    failed = any(str(v).strip().lower() == "failed" for v in values_list(task_data, "TaskStatus"))
    failed = failed or str(first_value(job_data, ("Status", "JobStatus"), "")).strip().lower() in {"failed", "error"}
    ...
    if not parts:
        parts.append(f"Deadline indicou erro no job {job_id}, mas não retornou relatórios.")
    return True, "\n\n".join(parts)
...
def convert_deadline_render_path(render_output):
    ...
    if os.path.splitext(base)[1] or any(token in base for token in ("#", "%0", "<FRAME>", "$F")):
...
def discover_deadline_job_ids():
    ...
    if result and output:
        ...
        log_and_print("⚠ Não foi possível listar jobs via -GetJobs True; usando apenas DEADLINE_JOB_IDS e jobs já vinculados.", "warning")
...
def process_deadline_job(cursor, p00_rollup=None, deadline_job_id=None, job_data=None, task_data=None, raw_job_output=""):
    is_deadline_job = bool(deadline_job_id)

    if not is_deadline_job:
        log_and_print("process_deadline_job recebeu chamada sem deadline_job_id; ignorado no monitor Deadline.", "warning")
        return
    ...
    has_error, errors = deadline_error_info(deadline_job_id, job_data or {}, task_data or {})
    job_folder = deadline_job_id
    folder_name = xml_data.get("Name") or deadline_job_id
    log_and_print(f"\nProcessando Deadline Job: {deadline_job_id} - {folder_name}")
    ...
    if linked_render:
        ...
        log_and_print(f"Render vinculado encontrado: render_id={linked_render[0]}, imagem_id={imagem_id}")
    else:
    ...
    status_id = linked_render[2] if linked_render and linked_render[2] else find_status_id(cursor, imagem_id)
...
def main():
    ...
    if conn is None:
        log_and_print("Conexao com banco indisponivel. Encerrando monitor Deadline.", "error")
        return

    with conn.cursor() as cursor:
        if not render_alta_has_deadline_job_id(cursor):
            log_and_print("A coluna render_alta.deadline_job_id nao existe no banco atual. Crie a coluna antes de ativar o monitor Deadline.", "error")
            return
        ...
        log_and_print(f"Jobs Deadline na fila: {len(queue)} (vinculados={len(linked_jobs)}, descobertos={len(discovered_job_ids)})")

        for job_id in queue:
            try:
                job_data, raw_job_output = get_deadline_job(job_id)
                if not job_data:
                    log_and_print(f"Job Deadline nao encontrado ou sem dados: {job_id}", "warning")
                    continue
                ...
                if MAX_JOBS and processed_jobs >= MAX_JOBS:
                    log_and_print(f"Limite MAX_JOBS atingido ({MAX_JOBS}). Encerrando processamento.")
                    break
            except Exception as e:
