    ...
    $sql = "SELECT 
      ...
      (
        ...
        WHERE fi_sub.funcao_id = fi.funcao_id
          ...
          AND MONTH(fi_sub.data_pagamento) = ? - 1
          AND YEAR(fi_sub.data_pagamento) = YEAR(CURDATE())
      ) AS quantidade_mes_anterior
    FROM funcao_imagem fi
    JOIN funcao f ON f.idfuncao = fi.funcao_id
    WHERE fi.valor > 0 AND fi.pagamento = 1 AND fi.data_pagamento <> '0000-00-00' 
      AND MONTH(fi.data_pagamento) = ? AND YEAR(fi.data_pagamento) = YEAR(CURDATE())
    ...
    $sql = "SELECT 
      ...
      (
        ...
        WHERE fi_sub.colaborador_id = fi.colaborador_id 
          AND MONTH(fi_sub.data_pagamento) = MONTH(CURDATE()) - 1
          AND YEAR(fi_sub.data_pagamento) = YEAR(CURDATE())
      ) AS quantidade_mes_anterior,
      (
