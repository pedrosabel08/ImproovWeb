...
async function enviarFormularioNotificacao(form) {
  ...
  try {
    ...
    salvarToastParaReload(
      json.message || "Operação concluída.",
      json.warning ? "warning" : "success",
    );
...
async function loadStatus(notificacaoId) {
  ...
  const res = await fetch(
    `actions/recipients.php?id=${encodeURIComponent(notificacaoId)}`,
  );
  ...
  if (total === 0) {
    tbody.innerHTML =
      '<tr><td colspan="4" class="small">Sem destinatários (verifique a segmentação).</td></tr>';
    return;
  }
...
async function abrirPreviaNotificacao(notificacaoId) {
  const response = await fetch(
    `actions/preview.php?id=${encodeURIComponent(notificacaoId)}`,
    { headers: { Accept: "application/json" } },
  );
  const json = await response.json().catch(() => ({}));
  if (!response.ok || !json.ok || !json.data)
    throw new Error(json.message || "Não foi possível carregar a prévia.");
  if (!window.FlowNotificationRenderer)
    throw new Error("Componente de prévia não carregado.");
...
function validarArquivosNotificacao() {
  ...
  let erro = "";
  if (files.length > NOTIFICACAO_MAX_ARQUIVOS)
    erro = `Envie no máximo ${NOTIFICACAO_MAX_ARQUIVOS} arquivos.`;
  const total = files.reduce((sum, file) => sum + file.size, 0);
  if (!erro && total > NOTIFICACAO_MAX_TAMANHO_TOTAL)
    erro = "O total dos arquivos não pode exceder 40 MB.";
  for (const file of files) {
    const ext = (file.name.split(".").pop() || "").toLowerCase();
    if (!erro && !NOTIFICACAO_EXTENSOES.includes(ext))
      erro = `Formato não permitido: ${file.name}.`;
    if (!erro && (file.size < 1 || file.size > NOTIFICACAO_MAX_TAMANHO_ARQUIVO))
      erro = `Cada arquivo deve ter até 10 MB: ${file.name}.`;
  }
  if (feedback)
    feedback.textContent =
      erro ||
      (files.length ? `${files.length} arquivo(s) selecionado(s).` : "");
  if (erro) input.value = "";
  return !erro;
}
...
document.addEventListener("click", async (e) => {
  ...
  if (statusBtn) {
    ...
    try {
      ...
      if (tbody)
        tbody.innerHTML =
          '<tr><td colspan="4" class="small">Erro ao carregar status.</td></tr>';
    }
    return;
  }
