...
function configurarPdfJs() {
  if (window.pdfjsLib?.GlobalWorkerOptions) {
    ...
    window.pdfjsLib.GlobalWorkerOptions.workerSrc =
      root + "/assets/pdfjs/pdf.worker.min.js";
  }
}
...
function resolveImproovUrl(path) {
  ...
  try {
    ...
    if (
      ...
      pageUseFlow &&
      /\/ImproovWeb(\/|$)/i.test(resolvedUrl) &&
      !/\/flow\/ImproovWeb/i.test(resolvedUrl)
    ) {
  ...
  if (!/^https?:\/\//i.test(resolvedUrl)) {
    ...
    if (resolvedUrl.startsWith("/")) {
      if (
        resolvedUrl.startsWith("/ImproovWeb") &&
        useFlow &&
        !resolvedUrl.startsWith("/flow/")
      ) {
      ...
      const base =
        (window.PROJECT_ROOT && String(window.PROJECT_ROOT).trim()) ||
        defaultRoot;
      resolvedUrl =
        base.replace(/\/+$/, "") + "/" + resolvedUrl.replace(/^\/+/, "");
    }
  }
  return resolvedUrl;
}
  ...
  switch (id) {
    ...
    case 1:
      return [
        ...
        "Filtro de assets",
        "Finalização P00",
      ]; // Todas menos finalização e pós-produção
    case 19:
...
sino.addEventListener("click", function () {
  ...
  const audio = new Audio(
    "https://improov.com.br/flow/ImproovWeb/sons/not.mp3",
  );
  audio.play();
...
function exibirAvisoUltimoDiaUtil() {
  ...
  fetch(
    `getFuncoesPorColaborador.php?colaborador_id=${colaborador_id}&ano=${ano}&mes=${mesFormatado}`,
  )
    .then((response) => response.json())
    .then((data) => {
      if (data.length === 0) {
...
function exibirModalPrimeiroDiaUtil() {
  ...
  radios.forEach((radio) => {
    radio.addEventListener("change", () => {
      observacaoDiv.style.display =
        radio.value === "alteracao" && radio.checked ? "block" : "none";
    });
  });
}
...
function abrirModalNotificacaoModulo(notificacao, onClose) {
  ...
  if (!modal) {
    ...
    modal
      .querySelector(".noti-modal__overlay")
  ...
  console.debug(
    "abrirModalNotificacaoModulo: abrindo notificação",
    notificacao.id,
  );
...
function confirmarNotificacaoModulo(id, onClose) {
  ...
  fetch(statusUrl, {
    ...
    .then((data) => {
      const ok =
        data &&
        (data.ok === true || data.success === true || data.result === true);
      if (ok) {
        if (typeof onClose === "function") onClose();
      } else {
        // não fechar se servidor não retornar ok
        console.warn(
          "confirmarNotificacaoModulo: servidor retornou não ok",
          data,
        );
      }
    })
