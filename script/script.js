...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function carregarTabela() {
    fetch("atualizarTabela.php")
      ...
      .then((data) => {
        ...
        document.getElementById("total-imagens").textContent =
          contadorStatusZero;
        document.getElementById("total-imagens-antecipada").textContent =
          contadorAntecipada;
      })
  ...
  document
    ...
    .addEventListener("click", function (event) {
      ...
      var camposVazios = Array.from(camposPrazo).filter(
        (input) => !input.value,
      );
  ...
  function atualizarFuncoes() {
    ...
    if (obraId) {
      fetch(
        `getFuncoesPorObra.php?obra_id=${obraId}&tipo_imagem=${tipoImagem}&antecipada=${antecipada_obra}`,
      )
        .then((response) => response.json())
        .then((data) => {
          ...
          if (!Array.isArray(data.funcoes) || data.funcoes.length === 0) {
            console.warn(
              "Nenhuma função encontrada para esta obra e tipo de imagem.",
            );
            data.funcoes = [
              {
  ...
  document
    .getElementById("obra-follow")
    .addEventListener("change", fetchFollowUpData);
  document
    .getElementById("status_imagem")
    .addEventListener("change", fetchFollowUpData);
  document
    .getElementById("tipo_imagem_follow")
    .addEventListener("change", fetchFollowUpData);
  document
    .getElementById("antecipada_follow")
    .addEventListener("change", fetchFollowUpData); // Adiciona evento para "Antecipada"
  ...
  function fetchFollowUpData() {
    ...
    if (obraId) {
      fetch(
        `followup.php?obra_id=${obraId}&status_imagem=${statusImagem}&tipo_imagem=${tipoImagem}&antecipada=${antecipada}`,
      )
        .then((response) => response.json())
        .then((data) => {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function carregarDados(colaboradorId = null) {
    ...
    const funcoesSelecionadas = Array.from(
      document.getElementById("funcaoSelect").selectedOptions,
    ).map((opt) => opt.value);
    const statusSelecionados = Array.from(
      document.getElementById("statusSelect").selectedOptions,
    ).map((opt) => opt.value);
    ...
    if (funcoesSelecionadas.length)
      url += `&funcao_id=${encodeURIComponent(funcoesSelecionadas.join(","))}`;
    if (statusSelecionados.length)
      url += `&status=${encodeURIComponent(statusSelecionados.join(","))}`;
      ...
      .then((data) => {
        ...
        Object.keys(statusMap).forEach((status) => {
          const coluna = document.getElementById(
            `kanban-${status.replace(/\s/g, "").toLowerCase()}`,
          );
          if (coluna) {
            ...
            cards.slice(0, mostrarLimite).forEach((item) => {
              ...
              if (status === "Em aprovação") {
                ...
                else if (item.status === "Aprovado com ajustes")
                  dotClass = "status-aprovado-ajustes";
  ...
  if (idusuario != 1 && idusuario != 2 && idusuario != 9) {
    ...
    colaboradorSelect.addEventListener("change", () =>
      carregarDados(colaboradorSelect.value),
    );
  }
  ...
  document
    .getElementById("obraSelect")
    .addEventListener("change", () => carregarDados());
  document
    .getElementById("funcaoSelect")
    .addEventListener("change", () => carregarDados());
  document
...
function filtrarTabela() {
  ...
  for (var i = 0; i < linhas.length; i++) {
    ...
    var tipoImagemColuna =
      linhas[i].getElementsByTagName("td")[4].textContent ||
      linhas[i].getElementsByTagName("td")[4].innerText;
    ...
    var statusObra =
      linhas[i]
        ...
        .getAttribute("data-status-obra") === "0"; // Acesse o status_obra pela célula
  ...
  fetch(imgPath)
    ...
    .then((blob) => {
      ...
      reader.onloadend = function () {
        ...
        doc.setTextColor(0, 0, 0);
        doc.text(
          doc.splitTextToSize(importantMessage, 180),
          14 + doc.getTextWidth(importante),
          currentY,
        );
        currentY += 10;
...
document
  .getElementById("copyColumnColab")
