...
(async () => {
  ...
  function broadcastEnvelope(envelope, diagnosticScope, channel, payload) {
    ...
    console.log(
      `[WS-DIAG][Node][${diagnosticScope}] broadcast channel=${channel} event=${payload && payload.event ? payload.event : "(sem event)"} sockets_connected=${counts.connected} sockets_open=${counts.open} sockets_sent=${sent}`,
    );
    return sent;
  }
  ...
  function broadcastBriefing(envelope, briefingId, channel, payload) {
    ...
    wss.clients.forEach((socket) => {
      if (
        ...
        socket.briefingRooms &&
        socket.briefingRooms.has(String(briefingId))
      ) {
        socket.send(envelope);
        sent++;
      }
    ...
    console.log(
      `[WS-DIAG][Node][Briefing] room=briefing:${briefingId} channel=${channel} event=${payload && payload.event ? payload.event : "(sem event)"} sent=${sent}`,
    );
  }
  ...
  async function subscribeBriefing(ws, ticket) {
    ...
    try {
      ...
      ws.briefingParticipant = {
        id: Number(claims.participant_id),
        scope: claims.scope || "CLIENTE",
      };
      ws.send(
        JSON.stringify({
          info: "briefing_subscribed",
          briefing_id: Number(claims.briefing_id),
        }),
      );
    } catch (err) {
    ...
    try {
      ...
      await sub.pSubscribe("pos_producao:*", (message, channel) => {
        try {
          ...
          const counts = socketCounts();
          console.log(
            `[WS-DIAG][Node][Pós] redis.received channel=${channel} event=${payload && payload.event ? payload.event : "(sem event)"} payload=${message} sockets_connected=${counts.connected} sockets_open=${counts.open}`,
          );
      ...
      await sub.pSubscribe("render:*", (message, channel) => {
        try {
          ...
          const counts = socketCounts();
          console.log(
            `[WS-DIAG][Node][Render] redis.received channel=${channel} event=${payload && payload.event ? payload.event : "(sem event)"} payload=${message} sockets_connected=${counts.connected} sockets_open=${counts.open}`,
          );
      ...
      await sub.pSubscribe("briefing:*", (message, channel) => {
        try {
          ...
          const briefingId = String(
            payload.briefing_id || channel.split(":")[1] || "",
          );
      ...
      console.log(
        `[WS-DIAG][Node] redis.subscribed REDIS_URL=${REDIS_URL} patterns=upload_progress:*,pos_producao:*,render:*,flow_review:*,fotografico:*,flow_block:*,briefing:*,funcao_atualizada:*`,
      );
      ...
      console.error(
        "[WS-DIAG][Node] redis.connect.error REDIS_URL=" + REDIS_URL,
  ...
  wss.on("connection", (ws) => {
    ...
    const counts = socketCounts();
    console.log(
      `[WS-DIAG][Node] socket.connected sockets_connected=${counts.connected} sockets_open=${counts.open}`,
    );
